...
def _parse_calculation_scalars(vasprun_path: Path) -> tuple[list[float], list[list[float]]]:
    ...
    for calculation in root.findall(".//calculation"):
        ...
        if energy_node is None:
            energy_node = calculation.find("./energy/i[@name='e_0_energy']")
        if energy_node is not None and energy_node.text is not None:
            total_energies.append(float(energy_node.text))

        stress_node = calculation.find("./varray[@name='stress']")
        if stress_node is not None:
            rows = stress_node.findall("./v")
            if len(rows) == 3 and all(row.text for row in rows):
                matrix = [[float(value) for value in row.text.split()] for row in rows]
                stresses.append(
                    [
                        matrix[0][0],
                        matrix[1][1],
    ...
    return total_energies, stresses
...
def parse_vasprun_to_mlab_configurations(vasprun_path: str | Path, ctifor: float | None = None) -> list[MLABConfiguration]:
    ...
    structures = list(read_vasp_xml(str(vasprun), index=slice(None)))
    total_energies, stresses = _parse_calculation_scalars(vasprun)

    n_configs = min(len(structures), len(total_energies), len(stresses))
    if n_configs == 0:
    ...
    for idx in range(n_configs):
        ...
        symbols, counts = _ordered_symbols_and_counts(atoms.get_chemical_symbols())
