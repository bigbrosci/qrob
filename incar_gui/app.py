...
# First, add categorized tasks from config file (these have priority)
...
# Then add remaining tasks from incar module (only if not already defined in config)
for key in tasks_incar.keys():
    # Skip if already added from config (check by looking for matching display name)
    ...
    # Only add if not already in TASK_MAPPING from config
    if readable_name not in [v['display'] for v in TASK_MAPPING.values()]:
        TASK_MAPPING[key] = {
